...
public class ContributorController : ControllerBase
{
    ...
    public async Task<ActionResult<List<ContributorPreviewDTO>>> GetContributorsByTitleBasic(string tconst)
    {
        var contributors = await _context.Contributors
          .Where(c => c.Tconst == tconst)
          .OrderBy(c => c.Priority)
        ...
        var dto = _mapper.Map<List<ContributorPreviewDTO>>(contributors);
        return Ok(dto);
    }
    ...
    public async Task<ActionResult<ContributorPreviewDTO>> GetContributorByIdBasic(string iconst)
    {
        var contributor = await _context.Contributors
          .Where(c => c.Iconst == iconst)
          .FirstOrDefaultAsync();
        ...
        var dto = _mapper.Map<ContributorPreviewDTO>(contributor);
        return Ok(dto);
    }
    ...
    public async Task<ActionResult<ContributorPreviewDTO>> GetContributorByTitleAndIdBasic(string tconst, string iconst)
    {
        var contributor = await _context.Contributors
          .Where(c => c.Tconst == tconst && c.Iconst == iconst)
          .FirstOrDefaultAsync();
    ...
    [HttpGet("all/basic")]
    public async Task<ActionResult<List<ContributorPreviewDTO>>> GetAllContributorsBasic([FromQuery] int page = 1, [FromQuery] int pageSize = 50)
    {
        page = page < 1 ? 1 : page;
        pageSize = pageSize < 1 ? 50 : pageSize;
